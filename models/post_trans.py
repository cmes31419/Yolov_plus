...
class MSA_yolov(nn.Module):
    ...
    def forward(self, x_cls, x_reg, cls_score=None, fg_score=None, sim_thresh=0.75, ave=True, use_mask=False,**kwargs):
        ...
        end_event.record()
        torch.cuda.synchronize()
        msa_time = start_event.elapsed_time(end_event) / 1000.0  # Convert to seconds
        print(f"    msa time (GPU): {msa_time:.6f}s")
        ...
        end_event.record()
        torch.cuda.synchronize()
        other_time = start_event.elapsed_time(end_event) / 1000.0  # Convert to seconds
